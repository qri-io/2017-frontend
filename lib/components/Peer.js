...
export default class Peer extends Base {
  ...
  componentWillMount () {
    if (!this.props.peerid) {
      this.props.loadPeerByName(this.props.peername).then((action) => {
        if (action.type === 'PEER_FAILURE') {
          this.setState({noPeer: true})
        }
      })
    } else if (!this.props.peer) {
      this.props.loadPeerById(this.props.peerid).then(() => {
  ...
  componentWillReceiveProps (nextProps) {
    if (this.state.noPeer) {
      return
    }
    if (!nextProps.peerid) {
      this.props.loadPeerByName(nextProps.peername).then((action) => {
        if (action.type === 'PEER_FAILURE') {
          this.setState({noPeer: true})
        }
      })
    } else if (!nextProps.peer || this.props.peerid !== nextProps.peerid) {
      this.props.loadPeerById(nextProps.peerid).then(() => {
  ...
  template (css) {
    const { peer, profile, peername, sessionPeer } = this.props
    const { tabIndex, noPeer } = this.state

    if (noPeer) {
      return <NoPeer onGoBack={this.handleGoBack} peername={peername} sessionPeer={sessionPeer} />
    }
