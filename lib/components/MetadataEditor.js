...
export default class MetadataEditor extends Base {
  ...
  template (css) {
    ...
    if (this.state.loading) {
      ...
      return (
        <div className={`metadata editor ${css('pageWrap')}`}>
          <div className={css('formWrap')}>
            <div className={css('headWrap')}>
              <div className='h1'>Metadata Editor</div>
              {showHelp ? <a onClick={this.handleClickHideHelp}>(hide help)</a> : <a onClick={this.handleClickShowHelp}>(show help)</a>}
            </div>
            <MetadataForm
