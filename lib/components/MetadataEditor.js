...
export default class MetadataEditor extends Base {
  ...
  componentWillMount () {
    ...
    if (!localDatasetRef && datasetRef) {
      const { name, path, dataset, peername } = this.props.datasetRef
      this.props.newDataset({name: name, path: path, peername: peername}, dataset, path)
    }
  }
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (!localDatasetRef && datasetRef) {
      const { name, path, dataset, peername } = nextProps.datasetRef
      nextProps.newDataset({name: name, path: path, peername: peername}, dataset, path)
    }
    if (localDatasetRef && datasetRef) {
  ...
  handleChange (name, value) {
    ...
    const updatedDataset = Object.assign(dataset, {meta: { [name]: value }})
