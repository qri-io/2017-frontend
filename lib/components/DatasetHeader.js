...
export default class DatasetHeader extends Base {
  ...
  renderHeader () {
    const { datasetRef, exportPath, onClickEdit, onClickDelete, onClickAdd, onGoBack, isLatestDataset, sessionProfile } = this.props
      ...
      <div className='dataSetHeader'>
        ...
        <div className={css('flex')}>
          <div className={css('datasetItem')}>
            <DatasetItem rename={onClickRename} data={datasetRef} link={false} peer={peer} isLatestDataset={isLatestDataset} />
          </div>
          <div className={css('buttons')}>
            { onClickDelete ? <Button text='Delete' onClick={onClickDelete} color='d' name='delete' /> : undefined }
            { onClickAdd ? <Button text='Add' onClick={onClickAdd} color='a' name='add' /> : undefined }
            { exportPath ? <Button text='Export' name='export' download={exportPath} /> : undefined }
          </div>
        </div>
      </div>
    )
