...
export default class ValidTextarea extends Base {
  template (css) {
    const { label, name, type, value, placeholder, showError, error, helpText, showHelpText, onChange, className, labelTop } = this.props
    ...
    return (
      <div className={(error && showError) ? `validFormField form-group has-error ${className}` : `validFormField form-group ${className}`}>
        <div className='form-control-wrap-lg'>
          <textarea
            id={name}
            name={name}
            type={type}
            className={`form-control ${css('textarea')} ${inputPosition}`}
            value={value}
