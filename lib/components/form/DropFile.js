...
export default class DropFile extends Base {
  ...
  handleChange (e) {
    ...
    } else if (e.target.files[0].name.endsWith('.csv') || e.target.files[0].name.endsWith('.json')) {
      this.handleAddFile(e.target.files, e)
    } else {
      this.handleResetDropZone('Dataset files must be in CSV or JSON format')
    }
  }
      ...
      if (dt.items) {
        if (dt.items.length > 1) {
          ...
          if (f.name.endsWith('.csv')) {
            ...
            this.handleResetDropZone('Dataset files must be in CSV format', e)
          }
        } else {
          this.handleResetDropZone('Can only upload files', e)
        }
      }
    } else {
      this.setState({error: ''})
  ...
  styles () {
    ...
    return {
      ...
      reset: {
        border: `2px dashed ${palette.neutral}`
      },
      dragOver: {
        border: `2px dashed ${palette.a}`
      },
      dropped: {
