...
export default class ValidCitationsInput extends Base {
  ...
  componentWillReceiveProps (nextProps) {
    console.log('ValidCitationsInput received props')
    console.log(this.props.citations)
    console.log(nextProps.citations)
  }

  shouldComponentUpdate (nextProps) {
    console.log(JSON.stringify(this.props.citations))
    console.log(JSON.stringify(nextProps.citations))
    return JSON.stringify(nextProps.citations) === JSON.stringify(this.props.citations)
  }

  handleAddRow () {
    const {citations} = this.props
    this.handleChange(citations.length, {})
  }

  handleChange (index, citation) {
    const { onChange, citations } = this.props
    var newCitations = citations
  ...
  handleRemoveRow (i) {
    const {onChange, citations} = this.props
    var newCitations = citations.slice(0, i).concat(citations.slice(i + 1))
  ...
  template (css) {
    const { citations, error, showError, helpText, showHelpText } = this.props
    return (
