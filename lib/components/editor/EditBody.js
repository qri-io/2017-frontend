...
import { generateSchemaFromRow, generateCollapsedChanges } from '../../utils/generate'
...
export default class EditBody extends Base {
  ...
  onAddBody () {
    ...
    newSchema = generateSchemaFromRow(body[0])
  ...
  onSetView (name, value) {
    ...
    if (value === 'table') {
      ...
      if (schema) {
        // case where the body is a 2D array, but the we have a schema &
      ...
      // case where the body is a 2D array and we have no schema
      // we need to generate one
      // and ensure that the columnOrder is set??
      const newSchema = generateSchemaFromRow(row)
      this.props.setSchema(newSchema)
      this.props.setColOrder(newSchema.items.items.map((elem, index) => index))
      this.props.setRowOrder(row.map((elem, index) => index))
      this.props.onSetView(value)
      return
    }
