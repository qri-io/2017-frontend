...
export default class EditBody extends Base {
  ...
  makeColHeaders () {
    ...
    const schema = extractSchema({ dataset: { structure } })
    const headerRow = structure && structure.formatConfig && structure.formatConfig.headerRow
    if (!schema && headerRow) {
      return []
    }
  ...
  renderEditor () {
    const { body, bodyString, structure, bodyView } = this.props
  ...
  template (css) {
    const { body, structure, bodyView } = this.props
      ...
      <div className={css('wrap')}>
        <header className={css('header')}>
          ...
          <div className={css('select')}>
            <ValidSelect label='view' name='view' options={['table', 'code']} onChange={this.onSetView} allowEmpty={false} labelTop value={bodyView} />
          </div>
          {bodyView === 'table' &&
          <div className={css('columnHeaders')}>
            <ValidCheck label='Column Headers' name='columnHeaders' onChange={(e) => console.log(e)} labelTop />
          </div>
          }
        </header>
