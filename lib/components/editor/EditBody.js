...
export default class EditBody extends Base {
  constructor (props) {
    ...
    [
      ...
      'onCollapseChanges',
      'handleSetBodyFile',
      'handleRemoveBodyFile'
    ].forEach((m) => { this[m] = this[m].bind(this) })
  }

  componentWillMount () {
    const { bodyView, structure, body: prevBody, columnHeaders, colOrder, rowOrder, bodyFileName } = this.props
    if (bodyFileName) {
  ...
  handleRemoveBodyFile (e) {
    var cont = confirm('Are you sure you want to remove this body file?')
    if (cont) {
      this.props.setBodyFile()
    }
  }
  ...
  renderEditor (css) {
    ...
    // TODO (ramfox): refactor to its own section
    if (bodyFileName) {
      return (
        <div className={css('fileBox')}>
          <div className={css('fileName')}>
            <div className={css('remove')} onClick={this.handleRemoveBodyFile}>x</div>
            {bodyFileName}
          </div>
        </div>)
    }
  ...
  template (css) {
    ...
    if (bodyFileName === '' && body === undefined) {
      return (
        ...
        <header className={css('header')}>
          <span style={{ float: 'right' }}>
            ...
            <input
              ...
              name='bodyFile'
              id='bodyFileUpload'
              onChange={this.handleSetBodyFile}
              ref={(input) => { this.fileUpload = input }} />
            <Button onClick={this.onRemoveBody} color='b' text='Remove' />
          </span>
          { // if there is a bodyFileName, we have a body file
            // and so we do not want to display the view selector
            !bodyFileName &&
