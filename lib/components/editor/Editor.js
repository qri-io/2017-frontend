...
export default class Editor extends Base {
  ...
  onRun () {
    ...
    const parsed = generateParsedBodyAndStructure(body, validDataset.structure, colOrder, rowOrder)
    ...
    // otherwise, parsed is an object with a parsed.schema and parsed.body
    // containing a well parsed schema and body
    if (!validDataset.structure) {
      validDataset.structure = {}
    }
    if (parsed.schema) {
      validDataset.structure['schema'] = parsed.schema
    }
    ...
    return { name, dataset: validDataset, transformScript, vizScript, body: parsed.body }
  }
  ...
  sections () {
    const { location, dataset, transformScript, vizScript, body, bodyFileName } = this.props
    ...
    return addActiveToLink([
      ...
      { name: 'body', link: 'body', faded: isEmpty(body) && !bodyFileName }
