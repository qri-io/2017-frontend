...
export default class Editor extends Base {
  ...
  template (css) {
    const { resultTab, datasetError, datasetMessage, results } = this.state
    const { match, name, dataset, transformScript, vizScript, body } = this.props
    // TODO - make use of setCommit func props
    const { setName, setMeta, setStructure, setTransform, setViz, setTransformScript, setVizScript, setBody, updateBody, id } = this.props
