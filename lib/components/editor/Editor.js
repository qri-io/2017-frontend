...
import { generateCollapsedChanges, generateValid } from '../../utils/generate'
...
export default class Editor extends Base {
  ...
  onRun () {
    ...
    var validDataset = Object.assign({}, dataset, { name })

    // TODO (ramfox):this needs a better name
    // basically it takes body and structure
    // and returns the valid, parsed body and structure (collapsing the
    // changes, if needed). Returns a string error if body or schema cannot
    // be parsed
    // might be replaced by generateParsedSchema and generateParsedBody
    // but babysteps
    const valid = generateValid(body, validDataset.structure, colOrder, rowOrder)

    // if valid is a string, that means an error has returned
    if (typeof valid === 'string') {
      alert(valid)
      return
    }

    // otherwise, valid is an object with a valid.schema and valid.body
    // containing a well parsed schema and body
    if (!validDataset.structure) {
      validDataset.structure = {}
    }
    if (valid.schema) {
      validDataset.structure['schema'] = valid.schema
    }

    // all datasets sent over from the api are in json
    // if they were previously made in csv, they will converted back to csv
    // once they are sent to the Qri backend
    if (validDataset.structure) {
      validDataset.structure['format'] = 'json'
    }

    if (valid.body) {
      // we need to pass along a stringified version of body!
      valid.body = JSON.stringify(valid.body)
    }

    this.setState({ datasetError: '', datasetMessage: '', results: undefined, loading: true })
    return { name, dataset: validDataset, transformScript, vizScript, body: valid.body }
  }
