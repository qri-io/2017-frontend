...
export default class Editor extends Base {
  ...
  onRun () {
    let { name, dataset, transformScript, vizScript, body, valid, columnHeaders, bodyView } = this.props
    ...
    if (bodyView === 'table') {
      const changes = this.onCollapseChanges()
      if (!dataset.structure) {
        dataset.structure = {}
      }
      dataset.structure['schema'] = changes.schema
      body = changes.body
    }
    if (body && dataset && dataset.structure && dataset.structure.format === 'csv') {
      ...
      if (headerRow) {
        ...
        if (columnHeaders) {
          columnHeaders.forEach(r => row.push(r.title))
        } else {
        }
        bodyParsed.push(row)
  ...
  onCollapseChanges () {
    const { colOrder, rowOrder } = this.state
    const { body, columnHeaders } = this.props
    if (!rowOrder || !Array.isArray(rowOrder) || !colOrder || !Array.isArray(colOrder)) {
      return
    }
              ...
              <Route
                ...
                render={props => (
                  <EditBody
                    ...
                    bodyParsable={bodyParsable}
                    setSchema={setSchema}
                    columnHeaders={columnHeaders}
                    onChangeSchema={setSchema}
                    onChangeStructure={setStructure}
                    setRowOrder={this.setRowOrder}
                    setColOrder={this.setColOrder}
                    rowOrder={this.state.rowOrder}
                    colOrder={this.state.colOrder}
                    onCollapseChanges={this.onCollapseChanges}
                  />
                )}
              />
              <Route
