...
export default class Editor extends Base {
  ...
  onDryRun () {
    const { name, dataset, transformScript, vizScript } = this.props
    this.setState({ loading: true })
    this.props.dryRunDataset(name, dataset, transformScript, vizScript).then((action) => {
      let results
  ...
  onRunAndNew () {
    const { name, dataset, transformScript, vizScript } = this.props
    this.setState({ loading: true })
    this.props.newDataset(name, dataset, transformScript, vizScript).then((action) => {
      let results
      if (action.response && action.response.result && action.response.result.meta) {
        if (action.response.entities && action.response.entities.datasetDryRuns) {
          results = action.response.entities.datasetDryRuns[Object.keys(action.response.entities.datasetDryRuns)[0]]
  ...
  onRunAndSave () {
    const { name, dataset, transformScript, vizScript } = this.props
    this.setState({ loading: true })
    this.props.saveDataset(name, dataset, transformScript, vizScript).then((action) => {
      let results
      if (action.response && action.response.result && action.response.result.meta) {
        if (action.response.entities && action.response.entities.datasetDryRuns) {
          results = action.response.entities.datasetDryRuns[Object.keys(action.response.entities.datasetDryRuns)[0]]
