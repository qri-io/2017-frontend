...
export default class DatasetItem extends Base {
  ...
  template (css) {
    const { data, link, small, isLatestDataset, rename, border } = this.props
    ...
    if (link) {
      return (
        <div className={`${border && 'border-bottom '}${css('wrap', small && 'small')}`} onMouseEnter={this.handleOnMouseEnter} onMouseLeave={this.handleOnMouseLeave} >
