...
export default class QueryItem extends Base {
  ...
  template (css) {
    const { data, onSelect, palette } = this.props
    ...
    return (
      <div className={css('wrap')} onMouseEnter={this.handleOnMouseEnter} onMouseLeave={this.handleOnMouseLeave}>
        <div className={css('displayInlineBlock')} >
          ...
          <Hash hash={data.path} style={{display: 'inline-block'}} muted short noPrefix />
          <div className={`${css('save')} ${css(saveButtonState)}`} onClick={funcs.onClickSave} ><DatasetName save name='Save' /></div>
