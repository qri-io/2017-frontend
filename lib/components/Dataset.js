...
export default class Dataset extends Base {
  ...
  componentWillMount () {
    const { peername, name, datasetRef, path, data, datapath, sessionPeer } = this.props
    if (!path) {
      this.props.loadDatasetByName(peername, name)
    } else if (!datasetRef) {
      this.props.loadDatasetByPath(path)
    } else if (sessionPeer && !data) {
      this.props.loadDatasetData(path, datapath)
    }
  }

  componentWillReceiveProps (nextProps) {
    const { peername, name, sessionPeer } = this.props
    ...
    } else if (sessionPeer && !nextProps.data && !nextProps.noData) {
  ...
  handleReload () {
    ...
    this.loadDatasetData(path, datapath)
  }
  ...
  handleAddDataset (path, name, peer) {
    return peer ? () => {
      this.props.addDataset(path, name, '', '').then(() => {
        this.props.loadDatasets(this.props.sessionPeer)
      })
    } : undefined
  }
  ...
  handleEditMetadata (peername, name, path, peer) {
    if (!peer) {
  ...
  renderData (css) {
    const { data, datasetRef, sessionPeer } = this.props
    if (sessionPeer) {
      return (
  ...
  renderTabPanel (css) {
    const { datasetRef, peername, name } = this.props
  ...
  template (css) {
    const { datasetRef, isLatestDataset, name, peername } = this.props
    ...
    const { dataset, path, peer } = datasetRef
          ...
          <div className={css('wrap')} >
            <div className={css('topPanel')} >
              <DatasetHeader
                ...
                onClickEdit={this.handleEditMetadata(peername, name, path, peer)}
                ...
                sessionPeer={this.props.sessionPeer}
              />
            </div>
