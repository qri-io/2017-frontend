...
export default class Dataset extends Base {
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (!nextProps.datasetRef) {
      this.setState({datasetloading: true}, () => this.props.loadDataset(this.props.path))
    } else if (nextProps.datasetRef !== this.props.datasetRef) {
      this.setState({datasetloading: false, loading: false})
  ...
  renderData (css) {
    ...
    // silly hack to get around linting errors
    if (css) {
      return (<div className={css('comingSoonWrap')}><p>Coming Soon!</p></div>)
    }
    ...
    if (datasetRef.dataset.structure && datasetRef.dataset.structure.format === 'csv') {
      return (
        <DatasetDataGrid
