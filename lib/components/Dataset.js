...
export default class Dataset extends Base {
  ...
  renderViz (peername, name) {
    return (
      <iframe style={{width: '100%', height: '100%', minHeight: 650, border: 'none'}} src={`http://localhost:2503/render/${peername}/${name}`} />)
  }
  ...
  renderDataset (css) {
    const { datasetRef, isLatestDataset, peername, name, peer, sessionProfileName, viewMode } = this.props
