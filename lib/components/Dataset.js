...
export default class Dataset extends Base {
  ...
  renderDataset (css) {
    const { datasetRef, isLatestDataset, peername, peer, sessionProfileName, name, sessionProfile } = this.props
    ...
    return (
      <div className={css('wrap')} >
        ...
        <div className={css('bottom')} >
          <Switch>
            {/*  example route for just now to make sure it's working :)  */}
            <Route path={`${match.path}/spinner`} component={Spinner} />
          </Switch>
        </div>
      </div>
    )
