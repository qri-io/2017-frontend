...
export default class Dataset extends Base {
  ...
  componentWillMount () {
    const { peername, name, datasetRef, path, body, bodypath, sessionProfile } = this.props
    if (!path) {
  ...
  renderBody (css) {
    const { body, datasetRef, sessionProfile } = this.props
    if (sessionProfile) {
      return (
        <div className={css('overflow')} >
          <Data
            ...
            onSetLoadingData={this.props.loadDatasetBody}
          />
        </div>
      )
    } else {
