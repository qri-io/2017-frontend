    ...
    return (
      <div className={css('wrap')}>
        <header>
          ...
          <Link to={'/dataset/add'} ><button className='btn btn-neutral-bold right'>Add</button></Link>
          <hr />
        </header>
        <div className={css('list')}>
          <div className={`${showStats ? 'col-md-8' : undefined}`}>
            ...
            {error ? <button className={`btn btn-neutral-bold ${css('reload')}`} onClick={this.handleReloadDatasets} >reload datasets</button> : undefined }
          </div>
        </div>
      </div>
    )
