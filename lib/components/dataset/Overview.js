...
export default class Overview extends Base {
  ...
  template (css) {
    const { datasetRef, profile, registryVersion, sessionProfile } = this.props
    ...
    return (
      <div className={css('wrap')}>
        ...
        {commit && <CommitItem data={datasetRef} profile={profile} index={0} registryVersion={sessionProfile && registryVersion} /> }
