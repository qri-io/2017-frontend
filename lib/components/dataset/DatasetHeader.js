...
class DatasetHeader extends Base {
  ...
  template (css) {
    const { url, datasetRef, fromRegistry } = this.props
    ...
    var linkList = datasetLinksAll.filter(elem => elem.link === 'overview' || (!fromRegistry && elem.link === 'body') || elem.link === 'history' || keys.includes(elem.link))
    if (!fromRegistry) {
      linkList.push({
        name: 'History',
        link: 'history'
