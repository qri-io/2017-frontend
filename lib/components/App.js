...
export default class App extends Base {
  ...
  renderModal (css) {
    if (this.props.modal) {
      return (
        <div id='modal-wrap'>
          <div className={`${css('modal')} dialog ${this.props.modal.large && 'large'} ${this.props.modal.thin && 'thin'}`} tabIndex='-1' role='dialog' onClick={(e) => { e.stopPropagation() }}>
  ...
  template (css) {
    const { children, layout, palette, apiConnection, location, user } = this.props
    ...
    return (
      <div className={css('app')} onClick={this.handleStageClick}>
        ...
        <Header />
        {this.props.sessionPeer ? <Menu palette={palette} location={location} user={user} style={layout.sidebar} /> : undefined}
        <div
          className='main'
          style={Object.assign({
            position: 'relative',
...
App.propTypes = {
  ...
  loadSessionPeer: PropTypes.func.isRequired,
