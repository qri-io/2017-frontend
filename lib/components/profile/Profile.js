      ...
      <div className={css('wrap')}>
        <div className={`sectionWidth border ${css('col1')}`} >
          ...
          <div className='sectionWidth' >
            {noProfile ? <NoProfile onGoBack={this.handleGoBack} peername={peername} sessionProfile={sessionProfile} /> : profile ? <ProfileBar profile={profile} peer={peer} /> : <Spinner />}
          </div>
        </div>
        ...
        <div className={`border-top ${css('col3')}`}>
          <div className='border-bottom headerHeight' />
        </div>
      </div>
    )
