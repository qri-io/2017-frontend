...
const body = {
  'data': [
    {
      'peername': 'ramfox',
      'name': 'epa_catalog',
      'peerID': 'QmPmyr6CXoVYLScXQ6MC9ZowSLG1oteAVs4GSbgawERTDE',
  ...
  'pagination': {
    'nextUrl': '/list/peerid?page=1\u0026pageSize=30'
  }
}
...
describe('Dataset Actions, loadDatasets:', () => {
  ...
  it('creates DATASET_SUCCESS when fetching datasets has been done', () => {
    ...
    var expectedActions = [
      { type: DATASETS_REQUEST, id: 'peerid', page: 1, pageSize: 30 },
      { type: DATASETS_SUCCESS, id: 'peerid', page: 1, pageSize: 30, response }
    ]

    return store.dispatch(loadDatasets(peerid)).then(() => {
