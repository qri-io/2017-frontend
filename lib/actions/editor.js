...
import { loadViz, loadTransform, loadDatasetBody } from './dataset'
  ...
  return (dispatch, getState) => {
    ...
    return new Promise(function (resolve, reject) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        // TODO - our API needs an "all" flag
        dispatch(loadDatasetBody(nes.dataset.path, nes.dataset.bodyPath, 1, 1000000)).then((action) => {
          if (action.response && action.response.entities && action.response.entities.body) {
            nes.body = action.response.entities.body[nes.dataset.bodyPath].data
          }
          resolve(nes)
        })
      })
...
export function setBodyFile (file, name) {
  console.log(file)
  return {
    type: EDITOR_SET_BODYFILE,
    file,
    name
  }
}
