...
export function loadDatasets (id, callback = () => {}, page = 1, pageSize = 30) {
  return (dispatch) => {
    ...
    return dispatch(
      fetchDatasets(id, page, pageSize)
      ).then(action => {
...
export function loadDatasetData (path, datapath, callback = () => {}, page = 1, pageSize = 100) {
  return (dispatch, getState) => {
    ...
    return dispatch(
      fetchDatasetData(path, datapath, page, pageSize)
    ).then(action => {
  ...
  return (dispatch, getState) => {
    return dispatch({
      ...
      if (action.type === DATASET_DELETE_SUCCESS) {
        ...
        // load datasets fresh
        const id = selectSessionPeer(getState())
