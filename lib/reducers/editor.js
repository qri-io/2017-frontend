...
export default function editorReducer (state = initialState, action) {
  switch (action.type) {
    ...
    case EDITOR_SET_BODYFILE:
      // if we have a bodyfile, we need to clear whatever body was there previously
      // TODO (ramfox): hey, we might not want to do this... but if we don't
      // then we run into a situation where we may be hosting two bodies
      // in the state tree, which has the potential to bloat v quickly
      // perhaps there is a world where we save the unsaved versions
      // some how, but for now, there is a warning that uploading
      // a bodyfile will remove any previous body
      //
      // setting bodyView to json, because that is our default
      // and what we want the user to see if they remove the body
      return Object.assign({}, state, {
        bodyFile: action.file,
        bodyFileName: action.name,
        body: undefined,
        colOrder: undefined,
        rowOrder: undefined,
        bodyView: 'json'
    ...
    case EDITOR_REMOVE_SECTION:
      const datasetSections = Object.keys(state.dataset).filter(s => {
        if (action.section === 'body') {
          return s !== action.section && s !== 'bodyBytes' && s !== 'bodyPath' && s !== state.bodyFile
        }
        return s !== action.section
      })
      dataset = {}
      datasetSections.forEach(s => { dataset[s] = state.dataset[s] })
      var { body, bodyFile, bodyFileName } = state
      if (action.section === 'body') {
        body = undefined
        bodyFile = undefined
      ...
      return Object.assign({}, state, { dirty: true, dataset, body, bodyFile, bodyFileName, colOrder, rowOrder })
