...
export function selectDatasetByName (state, peername, name) {
  ...
  // TODO - make this more robust. We shouldn't just assume that the
  // last added entity to namespace is the most up-to-date
  const id = Object.keys(namespace).reverse().find(id => (namespace[id].peername === peername && namespace[id].name === name))
  return namespace[id]
}
...
export function selectDataset (state, id) {
  ...
  if (!datasetRef) {
    datasetRef = state.entities.peerNamespace[id] ? Object.assign({}, state.entities.peerNamespace[id], {peer: true}) : undefined
  }
  return datasetRef
}
...
export function selectLocalDatasetByName (state, peername, name) {
  const { namespace } = state.locals
  const id = Object.keys(namespace).find(id => (namespace[id].peername === peername && namespace[id].name === name))
  return id ? namespace[id] : undefined
}
...
export function selectDatasets (state, section, node) {
  ...
  const { namespace } = state.entities
  function compareNamespace (a, b) {
    const timestampA = namespace[a].dataset.timestamp
    const timestampB = namespace[b].dataset.timestamp
    //
  ...
  return selectDatasetsIds(state, section, node, compareNamespace).map(id => namespace[id])
}
...
export function selectDatasetQueries (state, section, node) {
  // TODO
}
