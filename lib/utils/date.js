...
export function execTimeString (start, stop) {
  const seconds = (new Date(stop).valueOf() - new Date(start).valueOf()) / 1000
  ...
  if (seconds > 3600) {
    units = [parseInt(seconds / 60 / 60)].concat(units)
  }
  return units.join(':').replace(/\b(\d)\b/g, '0$1')
}
